  ...
  return (
    <div>
      ...
      >
        ...
        {invoice.entity && (
          <div className="text-center mb-5 pb-3" style={{ borderBottom: "2px solid #000" }}>
            ...
            {invoice.entity.address && (
              <p style={{ fontSize: "11px", color: "#000", marginTop: "4px" }}>{invoice.entity.address}</p>
            )}
            <div className="flex justify-center gap-4 mt-0.5" style={{ fontSize: "11px", color: "#000" }}>
            ...
            {invoice.entity.gstin && (
              <p className="font-mono mt-0.5" style={{ fontSize: "11px", color: "#000" }}>
        ...
        <div className="grid grid-cols-2 gap-6 mb-4 pb-3" style={{ borderBottom: "2px solid #000" }}>
          {invoice.customer && (
            <div>
              ...
              {invoice.customer.address && (
                <p style={{ fontSize: "11px", color: "#000", marginTop: "2px" }}>{invoice.customer.address}</p>
              )}
              {invoice.customer.phone && (
                <p style={{ fontSize: "11px", color: "#000" }}>Phone: {invoice.customer.phone}</p>
              )}
              {invoice.customer.gst_number && (
                <p className="font-mono" style={{ fontSize: "11px", color: "#000" }}>GSTIN: {invoice.customer.gst_number}</p>
              )}
            </div>
          )}
          <div className="text-right space-y-1">
            <div>
              <span style={{ fontSize: "10px", color: "#000", fontWeight: 600 }}>Invoice No.</span>
              <p style={{ fontSize: "13px", fontWeight: 700, color: "#000" }}>{invoice.invoice_number}</p>
            </div>
            <div>
              <span style={{ fontSize: "10px", color: "#000", fontWeight: 600 }}>Date</span>
              ...
              {items.map((item, index) => {
                ...
                return (
                  <tr key={item.id} style={{ borderBottom: "1px solid #000" }}>
                    <td className="py-1.5 px-2" style={{ color: "#000" }}>{index + 1}</td>
                    <td className="py-1.5 px-2" style={{ color: "#000" }}>{item.publisher || "-"}</td>
                    <td className="py-1.5 px-2" style={{ fontWeight: 500, color: "#000" }}>{item.title}</td>
                    <td className="py-1.5 px-2 text-right" style={{ color: "#000" }}>{item.quantity}</td>
                    <td className="py-1.5 px-2 text-right" style={{ color: "#000" }}>{item.price.toFixed(2)}</td>
                    <td className="py-1.5 px-2 text-right" style={{ color: "#000" }}>{grossAmt.toFixed(2)}</td>
                    <td className="py-1.5 px-2 text-right" style={{ color: "#000" }}>{item.discount.toFixed(2)}</td>
        ...
        <div className="mt-auto">
          ...
          {invoice.show_total !== false && (
            <table className="w-full mb-3" style={{ fontSize: "11px", borderCollapse: "collapse" }}>
              <tbody>
                ...
                {freight > 0 && (
                  <tr>
                    <td colSpan={5} className="py-1 px-2 text-right" style={{ color: "#000", fontWeight: 600 }}>Freight Charges</td>
                    <td className="py-1 px-2 text-right" style={{ fontWeight: 600, color: "#000" }}>{freight.toFixed(2)}</td>
                  </tr>
                )}
                {Math.abs(roundOff) > 0.001 && (
                  <tr>
                    <td colSpan={5} className="py-1 px-2 text-right" style={{ color: "#000", fontWeight: 600 }}>Round Off</td>
          ...
          {invoice.notes && (
            <div className="mb-3">
              <p style={{ fontSize: "11px" }}>
                ...
                <span style={{ color: "#000" }}>{invoice.notes}</span>
              </p>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4 mt-3 pt-2" style={{ borderTop: "1px solid #000" }}>
            {invoice.entity?.account_number ? (
              <div>
                ...
                <div style={{ fontSize: "9px", color: "#000", lineHeight: "1.4" }}>
                  {invoice.entity.bank_name && <p><span style={{ fontWeight: 600, color: "#000" }}>Bank Name:</span> {invoice.entity.bank_name}</p>}
            ...
            <div className="text-right">
              ...
              <p style={{ fontSize: "9px", color: "#000", borderTop: "1px solid #000", display: "inline-block", paddingTop: "2px", paddingLeft: "20px", paddingRight: "2px" }}>
          ...
          {/* Terms & Conditions */}
          <div className="mt-3 pt-2" style={{ borderTop: "1px solid #000" }}>
            <p style={{ fontSize: "10px", color: "#000" }}>
              <span style={{ fontWeight: 700 }}>Terms &amp; Conditions:</span> Goods once sold will not be taken back.
            </p>
          </div>
